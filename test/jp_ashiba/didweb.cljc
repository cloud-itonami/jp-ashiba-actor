...
(defn did->document-url
  "did:web の DID から、DID document が置かれているべき URL を導く。
   ...
   唯一の落とし穴で、この repo の 2 つの DID はちょうどその両側にある:
     ...
     did:web:jp-ashiba.etzhayyim.com        → /.well-known/did.json  （path 無し）
     did:web:etzhayyim.com:actor:jp-ashiba  → /actor/jp-ashiba/did.json（path 有り）
...
(defn resolution-url
  "claim の :did から :resolves-to を導く。did:web でも at:// でもなければ nil。"
  [did]
  (or (did->document-url did) (at-handle->did-url did)))
