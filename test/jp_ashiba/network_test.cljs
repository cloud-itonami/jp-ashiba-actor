...
(deftest each-claim-resolves-exactly-as-the-claims-file-records
  "測定値と実測が食い違ったら赤。**直った側にも落ちる** —— 例えば
   jp-ashiba.etzhayyim.com のホストが生えたら 0 → 200 で赤くなり、
...
(deftest the-did-that-resolves-returns-a-document-that-claims-that-same-did
  ...
  (if-not enabled?
    (println "   (skip: --network 無し)")
    (let [c (first (filter #(= :did/committed (:id %)) (:claims claims)))
...
(deftest the-resolving-document-is-not-the-one-this-repo-committed
  "**この repo の identity の要点。** id は解決するのに、解決先の document は
   commit したものと違う（commit 側は死んだ PDS を指している）。
  ...
  (if-not enabled?
    ...
    (let [c (first (filter #(= :did/committed (:id %)) (:claims claims)))
          ...
          committed (js->clj (js/JSON.parse (.readFileSync fs (path/join repo-root ".well-known/did.json") "utf8")))]
      (is (= (:serves-committed-bytes? (:measured c)) (= live committed))))))
...
(deftest the-pds-the-committed-document-points-at-is-still-the-dead-one
  ...
  (if-not enabled?
    (println "   (skip: --network 無し)")
    (let [committed (js->clj (js/JSON.parse (.readFileSync fs (path/join repo-root ".well-known/did.json") "utf8")))
          pds (->> (get committed "service")
                   (filter #(= "AtprotoPersonalDataServer" (get % "type")))
                   first
                   (#(get % "serviceEndpoint")))
          recorded (:url (first (filter #(= :pds/committed (:id %)) (:surfaces claims))))]
      (is (str/starts-with? recorded (str pds "/"))
